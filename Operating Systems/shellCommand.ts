...
module TSOS {
  ...
  export class DeviceDriverDisk extends TSOS.DeviceDriver {
    ...
    // -------- driver entry --------
    private krnDiskDriverEntry = (): void => {
      this.status = "disk driver loaded";
    };

    // ========== helpers ==========

    private findFreeDirectoryBlock(hd: Map<string, HardDriveBlock>): string | null {
    ...
    private findFreeStorageBlock(hd: Map<string, HardDriveBlock>): string | null {
      // data tracks = 1..3
    ...
    private convertStringToHex(str: string, padLength: number = 0): string {
    ...
    private getFileSize(startTSB: string, hd: Map<string, HardDriveBlock>): number {
      let size = 0;
      let currentTSB: string | null = startTSB;
    ...
    private getCreationDate(fileName: string): string {
      const metadata = (window as any)._FileMetadata?.[fileName];
    ...
    // shell "format"
    public format(_full: boolean = true): boolean {
    ...
    public createFile(fileName: string): boolean {
      ...
      // metadata
      const meta = ((window as any)._FileMetadata ||= {});
      if (!meta[fileName]) {
    ...
    public writeRawDataToFile(targetFileName: string, data: number[]): boolean {
      const hd = _HardDrive.getHardDriveMap();
      const paddedName = this.convertStringToHex(targetFileName, this.blockSize);
    ...
    // shell "read <name>"
    public readFile(targetFileName: string): string {
      const hd = _HardDrive.getHardDriveMap();
    ...
    // shell "ls" / "ls -a"
    public listFiles(showAll: boolean = false): Array<{ name: string; size: number; creationDate: string }> {
      const hd = _HardDrive.getHardDriveMap();
      const files: Array<{ name: string; size: number; creationDate: string }> = [];
    ...
    // ---- cp old new ----
    public copyFile(oldName: string, newName: string): boolean {
      const dataHex = this.readFile(oldName);
      if (!dataHex) return false;

      const text = this.convertHexToString(dataHex);

      if (!this.createFile(newName)) return false;
    ...
    public renameFile(oldName: string, newName: string): boolean {
      ...
      const text = this.convertHexToString(dataHex);
      if (!this.createFile(newName)) return false;
      if (!this.writeFile(newName, text)) return false;
    ...
    public findTSBByFilename(filename: string): string | null {
      const hd = _HardDrive.getHardDriveMap();
      const padded = this.convertStringToHex(filename, this.blockSize);
